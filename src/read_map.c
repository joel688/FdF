...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/12 19:26:29 by joakoeni          #+#    #+#             */
/*   Updated: 2023/07/06 13:39:20 by joakoeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	map_width(t_fdf *data)
{
	...
	while (clear_line[data->width] && clear_line[data->width][0] != '\n')
		data->width++;
...
void	colour_matrix(t_fdf *data)
{
	...
	i = 0;
	fd = open(data->map, O_RDWR);
	data->colour_matrix = malloc(sizeof(char **) * data->height + 1);
	while (data->height > i)
	{
		line = get_next_line(fd);
		clear_line = ft_split(line, ' ');
		data->colour_matrix[i] = malloc(sizeof(char *) * data->width + 1);
		fill_array(data, clear_line);
		free(line);
		free_split(clear_line);
		i++;
	}
	data->colour_matrix[i] = NULL;
	close(fd);
}

void	read_map(t_fdf *data, char *map)
{
	ft_printf("%sMap Loading...\n", "\e[36m");
	data->map = map;
	map_height(data);
	map_width(data);
	fill_matrix(data);
